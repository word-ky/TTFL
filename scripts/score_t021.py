"""Inherited T018 integer scoring after the complete T021 actual/matched freezes."""
...
from src.context.representation_gates import inherited_gates,regret_gate,diagnosis
...
def main():
    ...
    for rep in ('L','H'):
        for k,policy in enumerate(('oracle','source','clean_same_state')):states[rep,policy]=actual_data[rep]['choices'][k];prevalence[rep,policy]=actual_data[rep]['pi'][k]
    ...
    for rep in ('L','H'):
        for policy in ('oracle','clean_same_state'):actual_tables[rep,policy]=score_actual(rep,policy)
    ...
    ctx_rows={rep:paired_regret(rep,rep,'clean_same_state','context_vs_clean_same_state') for rep in ('L','H')}
    ...
    for rep in ('L','H'):
        ...
        g['REP_REGRET_A']=regret_gate(regret_rows[rep]);g['REP_CTX_A']=regret_gate(ctx_rows[rep]);gates[rep]=g
    ...
    save(out/'diagnostic_gates.json',dict(diagnosis=main_diagnosis,representations=gates,flags={r:dict(CTX='CTX+' if gates[r]['REP_CTX_A']['passed'] else 'CTX-',SRC=('SRC+' if gates[r]['REP_SRC_A'] else 'SRC-') if isinstance(gates[r]['REP_SRC_A'],bool) else 'NOT_EXECUTED') for r in ('L','H')}))
    ...
    save(out/'summary.json',dict(status='COMPLETE',runtime=a.commit,diagnosis=main_diagnosis,representations=gates,new_model_forwards=0,source_hashes_unchanged=True,seconds=time.time()-start,
        ...
        safety_inheritance='T018 real shifted >=BBSE-S-01 minus0.5pp; source clean >=zero minus0.5pp',source_scoring='only representations passing REP_REAL_A'))
    print('T021_SCIENCE_COMPLETE',json.dumps(load(out/'summary.json')),flush=True)
